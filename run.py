...
if __name__ == '__main__':
    ...
    raw_image = cv2.imread("./sample/06b6fc71-7743-5aa0-9ff0-1ac1a20499e2.jpg")[..., ::-1]
    ...
    for template_path in os.listdir("C:\\Users\\Bhushan\\Desktop\\RGIT_Hackathon\\robustTemplateMatching\\template"):
    
        raw_template = cv2.imread(os.path.join("C:\\Users\\Bhushan\\Desktop\\RGIT_Hackathon\\robustTemplateMatching\\template",template_path))[..., ::-1]
        template = image_transform(raw_template.copy()).unsqueeze(0)

        try:
            boxes, centers, scores = FE(template, image, threshold=None, use_cython=True)
        except:
            continue

        if(len(boxes)>0):
            d_img = raw_image.astype(np.uint8).copy()
            nms_res = nms(np.array(boxes), np.array(scores), thresh=0.9)
            ...
            if cv2.imwrite("result"+str(counter)+".png", d_img[..., ::-1]):
                print("result.png was generated")
                # print(boxes[0][0], boxes[0][1])
                # data = {
                #             args.image_path: [
                #                 args.template_path, [boxes[0][0][0],boxes[0][0][1], boxes[0][1][0], boxes[0][1][1]]
                #             ]
                #         }
